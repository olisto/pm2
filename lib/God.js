...
God.executeApp = function executeApp(env, cb) {
  ...
  // copy specified environment variables from the god process to the app
  if (env_copy['env_inherit']) {
    for (var i = 0; i < env_copy['env_inherit'].length; i++) {
      var key = env_copy['env_inherit'][i];
